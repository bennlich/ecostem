...
AnySurface.Scan = function() { return {
    ...
    setup: function(flatImgsrc, moundImgsrc, callbackA) {
        ...
        imgM.onload = imgF.onload = function() {
            ...
            console.log('image data set', !!this.imgData);
    ...
    chop: function(data, percent) {
        ...
        var thresh = Math.abs(max - min) * percent;

        for (var x = 0; x < data.length; ++x) {
            if (data[i] < min + thresh)
                data[i] = min + thresh;
            if (data[i] > max - thresh)
                data[i] = max - thresh;
        }
    ...
    find3D: function() {
        ...
        //this.differencesXY = this.chop(this.differencesXY, 0.3);
        //this.differencesXY = this.flood(this.differencesXY);
        if (blur) {
